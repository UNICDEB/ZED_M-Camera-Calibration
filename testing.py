...
# # Stable XYZ calculation using neighborhood
...
#             # Get stable XYZ
#             X, Y, Z = get_stable_xyz(point_cloud, click_x, click_y)

#             if X is not None:

#                 distance = np.sqrt(X**2 + Y**2 + Z**2)

#                 text = f"X:{X:.2f}m Y:{Y:.2f}m Z:{Z:.2f}m Dist:{distance:.2f}m"
...
# Stable XYZ using neighborhood median
def get_stable_xyz(point_cloud, x, y):
...
def main():
    ...
    global current_view, click_x, click_y
    ...
    cv2.namedWindow("ZED Live")
    cv2.setMouseCallback("ZED Live", mouse_callback)
    ...
    while True:
        ...
        if zed.grab(runtime) == sl.ERROR_CODE.SUCCESS:
            ...
            # Only calculate if mouse clicked
